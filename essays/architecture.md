...
- **Training-time circuit regularization and attention supervision.** Beyond post-hoc circuit discovery, prior work has explored using auxiliary objectives during training to force structural legibility. Geiger et al. (2021) introduce *Interchange Intervention Training (IIT)*, proving that causal intervention losses can induce standard neural networks to implement explicit, modular causal graphs. Similarly, Zhao (2024) demonstrates that factorized Mixture-of-Experts routing penalties can isolate specialized functional sub-networks, while Chung & Laletin (2026) and Raghavan (2026) show that regularizing attention matrices directly encourages structural explainability. These approaches provide empirical precedent for our *coordinating prior* ($\lambda$, §11), demonstrating that training-time structural pressure can induce modular, externalizable representations.
