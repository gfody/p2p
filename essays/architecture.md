...
2. **Research hypotheses** (Section 33, H1–H10). Trained networks contain comparatively compact relational structure — organized around a small number of primitive categories, exhibiting recurring algebraic patterns, generalizing under a weak interoperability objective, and potentially discoverable by relational variants of sparse autoencoders — that can become a shared computational vocabulary. A separate pair of hypotheses (H8, H10) concerns the coordinating prior's network-level dynamics rather than the algebraic structure itself: H8 asks whether the specific regime trade-offs at different $\lambda$ settings generalize as expected, and H10 asks whether attribution-linked incentive is enough to sustain participation. Both presuppose that the coordinating prior induces some effect in the first place — whether it does, and whether that effect is the grounded, compositional kind this architecture needs, is the more fundamental question posed and tested as P1 and P2 in the companion paper [*The Pressure*](the-pressure.md). These are stated as testable and (except for H7) directly falsifiable hypotheses.
...
| Tunable coordinating prior ($\lambda$) | Yes, as a mechanism — whether it induces the intended structure at all is P1/P2 ([*The Pressure*](the-pressure.md#8-pressure-works-in-two-directions)); its network-level regime effects are H8, its incentive dynamics are H10 | Exists architecturally regardless; how well it works, and whether participation is rational, are research questions (§11, §36) |
...
| Coordinating prior ($\lambda$) | Auxiliary training objective encouraging externalizable structure; tunable, default weak, primarily node-local (Section 11) |
...
**Content-addressable, not deduplicated.** Crowns are indexed by canonical form, which lets the network recognize when two submissions describe the same structure. That recognition is not a mandate to merge them. Two nodes can independently discover a functionally identical Crown, at different times, under different $\lambda$ regimes, and each retains its own provenance record; collapsing both into a single registry entry would silently erase one node's attribution as a side effect of hashing. Independent convergence on the same canonical form is closer to corroborating evidence than to redundant work — it is one of the few ways this architecture gets anything resembling replication (§12) — so the registry treats a second arrival at a known canonical form as a distinct, provenance-bearing event, not a duplicate to be discarded.
...
This alignment also serves a concrete coordination function. Rooting data into the network (§9) is not primarily a labeling task (what the data is) but an evaluative one (how and why it matters to the contributor). Such judgments scale across independent participants only if those participants share a coarse common basis for comparing relevance. The six categories supply that basis on the human side of the membrane, playing a role analogous to the coordinating prior $\lambda$ on the machine side (§11). Neither forces convergence on a single correct account; both give independently operating agents a shared reference frame in which their contributions can interoperate.
...
The architecture therefore introduces a tunable **coordinating prior** — a persistent training-time pressure for local computation to become expressible through the shared language, without forcing internal representations to converge.

> The membrane is the boundary. The coordinating prior is the pressure that makes useful structure cross it.

The mechanism-level case for why such a pressure should work — the structure/residual/provenance decomposition, the compositional-pressure and grounded-evolution hypotheses, and its relationship to compression, MDL, and emergent-communication research — is argued in full in the companion paper [*The Pressure*](the-pressure.md), and is not re-derived here. What follows is specific to how the prior functions as a network-facing control surface: something every node runs locally, tunes independently, and no other node can see or compel.
...
$\lambda$ controls the strength of interoperability pressure relative to the node's primary objective, which remains dominant. The prior does **not** require internal representations to converge; it biases how discoveries are externalized, not what internal representation a node arrives at.
...
That the dial exists, and that its authority is node-local, are architectural commitments stated with the same confidence as the rest of Sections 1–10. The *specific effects* attributed to each regime above — reduced diversity under sustained high $\lambda$, increased bottleneck risk, eventual collapse toward federated weight alignment — are not yet measured in this system. They are anticipated by analogy to known regularization and multi-task learning tradeoffs elsewhere in machine learning, which is why they are framed as expectations rather than guarantees; they are treated formally as a separable hypothesis in Section 33 (H8). Whether the pressure itself succeeds at inducing grounded, compositional structure in the first place is P1 and P2 in the companion paper [*The Pressure*](the-pressure.md) — a deeper question this architecture inherits rather than resolves.
...
The tradeoffs attributed to the coordinating prior's strength (§11) — that raising $\lambda$ increases collective utility and extraction ease at the cost of local diversity, and that lowering $\lambda$ protects sovereignty and residual novelty at the cost of slower sharing — generalize from established regularization and multi-task learning tradeoffs elsewhere in machine learning. This hypothesis presupposes that the coordinating prior has some effect on externalizability at all; whether it does, and whether that effect is the grounded, compositional kind rather than an arbitrary one, is tested separately as P1 and P2 in [*The Pressure*](the-pressure.md). H8 concerns only whether the specific regime-level trade-offs match what similar training pressures produce elsewhere in machine learning — the training dynamics of the coordination mechanism, not the algebraic structure of what gets coordinated. It is also the architecture's explicit hedge against mechanistic interpretability remaining immature indefinitely — if post-hoc circuit extraction does not mature on a useful timeline, raising $\lambda$ offers a fallback path toward externalizable structure that does not depend on that research program succeeding.
...
- **The learning-theoretic case for the coordinating prior.** Why training pressure should be expected to organize learned computation around grounded, compositional, externally legible structure — rather than merely producing behaviorally correct but opaque implementations — is a question this document treats as settled by architectural decision but does not itself argue from first principles. That argument, and its grounding in the interpretability, training-time-regularization, symbolic-approximation, compression/MDL, and emergent-communication literatures (Geiger et al. 2021; Chung & Laletin 2026; Raghavan et al. 2026; McCoy, Soulos, Linzen & Smolensky 2026; Hinton & van Camp 1993; Vereshchagin & Vitányi 2004; Foerster et al. 2016; Lazaridou & Baroni 2020), is made in full in the companion paper [*The Pressure*](the-pressure.md), which this document does not re-litigate.
...
- **Collective lifelong learning at the edge.** Soltoggio et al. (*Nature Machine Intelligence* 6, 251–264, 2024; doi:10.1038/s42256-024-00800-2) present the nearest existing technical vision of a society of edge agents that learn continuously over a lifetime and improve through knowledge sharing. Their Shared Experience Lifelong Learning (ShELL) framing integrates lifelong/continual learning, federated and decentralized methods, multi-agent systems, and edge deployment, and correctly identifies resilience, diversity of exploration, and reduced central training cost as central benefits. The unit of collaboration in that line of work remains knowledge that improves local models—parameters, gradients, representations, or distilled experience. The present architecture shares the high-level goal of a decentralized, lifelong collective but changes the object that crosses the network: validated, contract-bearing, provenance-tracked structure (Crowns expressed through a Neural Intermediate Representation) rather than model-derived updates. Interoperability is obtained through a semantic membrane and a tunable coordinating prior rather than by encouraging internal representational alignment, and admission is governed by Proof-of-Compression. Several of the open questions raised by Soltoggio et al.—selection of what and when to share, relevance at scale, and integration without catastrophic forgetting—remain relevant as implementation concerns once portable structure exists, but they are no longer the primary framing.
