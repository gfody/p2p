...
A model may discover a regularity that was not previously represented. Initially that regularity exists only as internal computation or residual structure; it may then become a candidate abstraction, a named or otherwise addressable structure, a structure with recorded provenance, a validated addition to the membrane, and finally a reusable component for subsequent learning. Conceptually, a new structure $m' = \text{compose}(m_1, \ldots, m_k)$ may arise from existing structures — but machine discovery need not always be compositional in this sense, and a genuinely novel primitive may emerge as well. The requirement is not that every new abstraction reduce to an existing human category. It is that its emergence remain traceable: grounding means grounded *extension*, not fidelity to a frozen ontology. The goal is not to force machine intelligence to remain inside the concepts humans already possess — it is to give machine discovery a grounded place from which to extend them.

---

### 6. Multiple Perspectives Are Part of the Structure

A shared semantic object need not have a single canonical interpretation. Different observers may legitimately expose different structures around the same source, and their differences need not be contradictions — they may simply occupy different relational projections of the same underlying structure. As demonstrated in the [handbook's worked example, *the dress*](digital-yogi-handbook.md#iii-a-worked-example-the-dress). The semantic membrane should preserve that multiplicity rather than prematurely collapsing it — which is one reason provenance is fundamental here: it is what lets several partial, non-contradictory descriptions of the same object coexist and still compose into something richer than any one of them alone.

---

### 7. The Coordinating Prior

The semantic membrane provides the substrate; the **coordinating prior** provides the pressure. These are distinct mechanisms, and it is important to separate three variables that are easy to conflate:

$$\boxed{M = \text{what structure is available (the reference frame)}}$$

$$\boxed{\mathcal{L}_{\mathrm{coord}} = \text{what organizational properties are pressured}}$$

$$\boxed{\lambda = \text{how strongly those properties matter}}$$

A conventional training objective optimizes for task-specific performance via $\mathcal{L}_{\mathrm{task}}(\theta)$. The coordinating prior adds a functional term $\mathcal{L}_{\mathrm{coord}}(\theta, M)$ that measures the degree to which learned computation can be represented as **grounded, reusable, compositional, and provenance-preserving structure relative to $M$**:

$$\mathcal{L}(\theta; M) = \mathcal{L}_{\mathrm{task}}(\theta) + \lambda \mathcal{L}_{\mathrm{coord}}(\theta, M)$$

The overall object being optimized is the parameter configuration that balances task execution with structural legibility:

$$\theta^* = \arg\min_{\theta} \left[ \mathcal{L}_{\mathrm{task}}(\theta) + \lambda \mathcal{L}_{\mathrm{coord}}(\theta, M) \right]$$

### 7.1 Formal Definition vs. Loss Implementation

Crucially, this formulation distinguishes between the **definition of the prior** and the **differentiable implementation of its loss**. The exact mathematical realization of $\mathcal{L}_{\mathrm{coord}}$ remains an empirical research question. Rather than asserting a single fixed loss formula, we define a **family of coordinating priors** parameterized by distinct structural pressures:

$$\mathcal{L}_{\mathrm{coord}} = \alpha L_{\mathrm{ground}} + \beta L_{\mathrm{reuse}} + \gamma L_{\mathrm{composition}} + \delta L_{\mathrm{provenance}} + \eta L_{\mathrm{causal}} + \dots$$

This decomposition is a structural breakdown of the hypothesis, not a final training objective. It establishes the hypothesis tightly enough that competing implementations—whether auxiliary losses, architectural constraints, or structural optimization algorithms—can be directly tested against it.

The prior is therefore less like a lens than a spring. A lens attempts to reveal structure that already exists; a spring changes the forces acting on the system so that different structures become more likely to develop in the first place.
